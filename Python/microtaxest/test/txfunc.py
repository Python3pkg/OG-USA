'''
------------------------------------------------------------------------
This script defines the functions thatread in data generated from the
OSPC Tax Calcultor and the 2009 IRS PUF. It then estimates tax functions
tau_{s,t}(x,y), where tau_{s,t} is the effective tax rate for a given
age (s) in a particular year (t), x is total labor income, and y is
...
def get_TaxFunParams(params):
    ...
    numparams   = integer > 0, number of parameters to estimate for the
                  tax function for each age s and year t
    desc_data   = boolean, =True if print descriptive stats for each age
                  s and year t
    graph_data  = boolean, =True if print 3D graph of data for each age
                  s and year t
    graph_est   = boolean, =True if print 3D graph of data and estimated
                  tax function for each age s and year t
    ...
    param_arr   = (s_max-s_min+1 x tpers x numparams) array, parameter
                  values for the estimated tax function for each age s
                  and year t
    AvgInc      = (tpers,) vector, average income in each year
    cmap1       = matplotlib setting, set color map for 3D surface plots
    years_list  = [beg_yr-end_yr+1,] vector, iterable list of years to
                  be forecast
    data_folder = string, path of hard drive folder where data files
                  reside
    start_time  = scalar, current processor time in seconds (float)

    This function calls the following functions:
        gen_etr_grid:   generates summary grid points for effective tax
                        rate
    ...
    return: dict_tfparams
    --------------------------------------------------------------------
    '''
    (s_min, s_max, beg_yr, end_yr, tpers, numparams, desc_data,
        graph_data, graph_est, dmtrgr_est) = params
    ...
    data_folder = \
        '/Users/rwe2/Documents/Economics/OSPC/Data/micro-dynamic/'
    ...
    for t in years_list:
        ...
        ----------------------------------------------------------------
        t          = integer >= 2015, current year of tax functions
                     being estimated
        ...
        data_trnc  = (I2 x 6) DataFrame, I2 observations (I2<I), 6 vars
        min_age    = integer, minimum age in data_trnc that is at least
                     s_min
        max_age    = integer, maximum age in data_trnc that is at most
                     s_max
        ages_list  = [age_max-age_min+1,] vector, iterable list of
                     relevant ages in data_trnc
        ...
        data_trnc = \
            data.drop(data[data['Effective Tax Rate'] >0.5].index)
        ...
        data_trnc = \
            (data_trnc.drop(data_trnc[data_trnc['Adjusted Total income']
            < 5].index))
        ...
        for s in ages_list:
            ...
            df_trnc      = (I5 x 6) DataFrame, truncated data for
                           parameter estimation
            inc_lab      = (I5 x 1) vector, total labor income for
                           parameter estimation
            ...
            varmat_bar   = (6,) vector, mean values of each column of
                           varmat
            ...
            phi          = (Obs,) vector, percent of total income that
                           is labor income x/(x+y) for each observation
            ...
            params_til   = length 8 tuple, output from minimizer. Object
                           with estimated param values is params_til.x
            ...
            max_x        = scalar > 0, estimated value for max_x (labor
                           income)
            min_x        = scalar, estimated value for min_x (labor
                           income)
            max_y        = scalar > 0, estimated value for max_y
                           (capital income)
            ...
            P_den        = (Obs,) vector, value of the denominator of
                           the ratio of polynomials for each observation
            etr_est      = (Obs,) vector, predicted effective tax rates
                           for each observation
            params       = (10,) vector, estimated parameters
                           (A, B, C, D, E, F, max_x, min_x, max_y,min_y)
            A            = scalar > 0, estimated polynomial coefficient
                           on x**2
            B            = scalar > 0, estimated polynomial coefficient
                           on y**2
            C            = scalar > 0, estimated polynomial coefficient
                           on x*y
            D            = scalar > 0, estimated polynomial coefficient
                           on x
            E            = scalar > 0, estimated polynomial coefficient
                           on y
            elapsed_time = scalar > 0, seconds to finish computation
            dict_params  = length 3 dictionary, saves param_arr, AvgInc,
                           and elapsed_time
            ------------------------------------------------------------
            '''
            print("year=", t, "Age=", s)
    ...
    dict_tfparams = dict([('tfunc_params', param_arr), ('tfunc_avginc',
